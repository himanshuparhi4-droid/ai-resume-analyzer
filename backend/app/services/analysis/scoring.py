...
class ScoringEngine:
    ...
    def _resume_quality_score(self, resume_data: dict) -> float:
        ...
        base += min(14, verb_hits * 2.5)
        quantified_hits = len(re.findall(r"\b\d+%|\b\d+[kKmM]?\b", text))
        base += min(12, quantified_hits * 2)
        if sections.get("summary"):
            base += 5
        if parse_signals.get("skills_count", 0) >= 6:
            base += 6
        if parse_signals.get("quantified_line_count", 0) >= 3:
            base += 6
        elif parse_signals.get("quantified_line_count", 0) >= 1:
            base += 3
        if parse_signals.get("bullet_like_line_count", 0) >= 3:
            base += 4
        ...
        penalties = (
            min(6, parse_signals.get("merged_header_count", 0) * 3)
            + min(4, parse_signals.get("inline_header_count", 0) * 1)
            ...
            + (3 if parse_signals.get("inferred_skills_section") else 0)
            + (3 if sections.get("experience") and parse_signals.get("quantified_line_count", 0) == 0 else 0)
            + (2 if sections.get("experience") and parse_signals.get("date_range_count", 0) == 0 else 0)
        )
        if parse_signals.get("multi_column_detected") and parse_signals.get("explicit_header_count", 0) >= 4:
            penalties = max(0, penalties - 3)
        return float(max(40, min(base - penalties, 92)))
    ...
    def _ats_score(self, resume_data: dict) -> float:
        ...
        score += len(required_sections & set(sections.keys())) * 12
        ...
        if 200 <= len(text.split()) <= 1100:
            score += 5
        if parse_signals.get("explicit_header_count", 0) >= 3:
            score += 12
        ...
        penalties = (
            ...
            + min(6, parse_signals.get("inline_header_count", 0) * 1.5)
            + min(8, parse_signals.get("section_leakage_count", 0) * 4)
            + min(6, parse_signals.get("suspicious_token_count", 0) * 1)
            + min(6, parse_signals.get("suspicious_url_count", 0) * 3)
            + (6 if parse_signals.get("inferred_skills_section") else 0)
            + (3 if sections.get("experience") and parse_signals.get("date_range_count", 0) == 0 else 0)
            + (2 if parse_signals.get("contact_link_count", 0) == 0 else 0)
        )
        return float(max(35, min(score - penalties, 94)))
